...
MEME_ALL = Data("memeall", "/x/lucene/data/memetracker/lines.txt", 210999824, constants.WIKI_MEDIUM_TASKS_10MDOCS_FILE)
...
DISJUNCTION_SIMPLE = Data("disjunctionSimple", constants.DISJUNCTION_DOCS_LINE_FILE, constants.DISJUNCTION_DOC_COUNT, constants.DISJUNCTION_SIMPLE_TASKS_FILE)

DISJUNCTION_REALISTIC = Data("disjunctionRealistic", constants.DISJUNCTION_DOCS_LINE_FILE, constants.DISJUNCTION_DOC_COUNT, constants.DISJUNCTION_REALISTIC_TASKS_FILE)

DISJUNCTION_INTENSIVE = Data("disjunctionIntensive", constants.DISJUNCTION_DOCS_LINE_FILE, constants.DISJUNCTION_DOC_COUNT, constants.DISJUNCTION_INTENSIVE_TASKS_FILE)

COMBINED_FIELDS_BIG = Data("combinedFieldsBig", constants.WIKI_BIG_DOCS_LINE_FILE, constants.WIKI_BIG_DOCS_COUNT, constants.COMBINED_FIELDS_TASKS_FILE)

COMBINED_FIELDS_UNEVENLY_WEIGHTED_BIG = Data(
  "combinedFieldsUnevenlyWeightedBig", constants.WIKI_BIG_DOCS_LINE_FILE, constants.WIKI_BIG_DOCS_COUNT, constants.COMBINED_FIELDS_UNEVENLY_WEIGHTED_TASKS_FILE
)

COMBINED_FIELDS_MEDIUM_10M = Data("combinedFieldsMedium10M", constants.WIKI_MEDIUM_DOCS_LINE_FILE, 10000000, constants.COMBINED_FIELDS_TASKS_FILE)

COMBINED_FIELDS_UNEVENLY_WEIGHTED_MEDIUM_10M = Data("combinedFieldsUnevenlyWeightedMedium10M", constants.WIKI_MEDIUM_DOCS_LINE_FILE, 10000000, constants.COMBINED_FIELDS_UNEVENLY_WEIGHTED_TASKS_FILE)
...
def sourceData(key=None):
  if not key:
    raise RuntimeError('Data source required for benchmark run. Use "-s/-source/--source" option to provide data source. \nValid options: %s' % DATA.keys())
  if key in DATA:
    return DATA[key]
...
class Index:
  ...
  def getName(self):
    ...
    name = [self.dataSource.name, benchUtil.checkoutToName(self.checkout)]
...
class Competitor:
  ...
  def __init__(
    ...
    if False and searchConcurrency != 0 and numConcurrentQueries > 1:
      raise RuntimeError(
        f"when searchConcurrency != 0 (got: {searchConcurrency}) you must specify numConcurrentQueries = 1 (got: {numConcurrentQueries}), otherwise you might see falsely low effective QPS"
      )
