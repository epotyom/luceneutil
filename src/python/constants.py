...
# TODO: this file and path is generated by ./gradlew vectors-mpnet ,
#       should we move it to data dir? And change generation script to do the same?
VECTORS_DOCS_FILE = "%s/util/tasks/enwiki-20120502-lines-1k-mpnet.vec" % BASE_DIR
...
if "JAVA_COMMAND" not in globals():
  # TODO: I've tried 2g and 4g and run into not enough heap, is 6g enough/too much?
  JAVA_COMMAND = "%s -server -Xms2g -Xmx6g --add-modules jdk.incubator.vector -XX:+HeapDumpOnOutOfMemoryError -XX:+UseParallelGC" % JAVA_EXE
else:
...
NIGHTLY_BIG_INDEX_NUM_DOCS = 5982049
...
# These two values are arbitrary - IDK what it is supposed to be
HNSW_THREADS_PER_MERGE = 2
HNSW_THREAD_POOL_COUNT = 2
# According to this? #219
VECTORS_QUERY_FILE = "%s/data/cohere-wikipedia-queries-768d.vec" % BASE_DIR
VECTORS_DIMENSIONS = 768
# I guess it's downloaded manually from here?
# http://download.geonames.org/export/dump/allCountries.zip
# TODO: download and unzip in initial_setup.py ?
